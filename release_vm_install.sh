...
cd /home/zafin
wget http://muug.ca/mirror/apache-dist/maven/maven-3/3.5.2/binaries/apache-maven-3.5.2-bin.tar.gz && \
                tar -zxf apache-maven-3.5.2-bin.tar.gz -C /usr/local/apache-maven

rm apache-maven-3.5.2-bin.tar.gz
export M2_HOME=/usr/local/apache-maven/apache-maven-3.5.2
export M2=$M2_HOME/bin
ln -s $M2/mvn /usr/bin/mvn
cp $M2_HOME/conf/settings.xml $M2_HOME/conf/settings-bac.xml && setup_maven_settings > $M2_HOME/conf/settings.xml $2 $3
